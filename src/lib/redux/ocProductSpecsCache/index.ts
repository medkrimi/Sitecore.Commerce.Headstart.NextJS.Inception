...
import {Spec, Me, RequiredDeep, Products} from "ordercloud-javascript-sdk"
...
export const ocProductSpecsAdapter = createEntityAdapter<
  RequiredDeep<ProductSpecsResult>
>({
  selectId: (p) => p.productId
})

export const ocProductSpecsCacheSelectors =
  ocProductSpecsAdapter.getSelectors<OcRootState>((s) => s.ocProductSpecsCache)

export const getProductSpecs = createOcAsyncThunk<
  RequiredDeep<ProductSpecsResult>,
  string
>("ocProductSpecs/get", async (productId, ThunkAPI) => {
  let productSpecs = ocProductSpecsCacheSelectors.selectById(
    ThunkAPI.getState(),
    productId
  )
  if (!productSpecs) {
...
const ocProductSpecsCacheSlice = createSlice({
  ...
  reducers: {
    ...
    cacheProductSpecs: (
      state,
      action: PayloadAction<RequiredDeep<ProductSpecsResult>>
    ) => {
...
export const {cacheProductSpecs, cleanProductSpecsCache} =
  ocProductSpecsCacheSlice.actions
